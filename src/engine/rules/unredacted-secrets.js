...
export function checkUnredactedSecrets(workflow, rawContent, filename) {
  ...
  function addFinding(contextLabel, lineNumber, label, id) {
    ...
    findings.push({
      ...
      severity: 'high',
      title: 'Secrets Passed Through Transform — Log Redaction Bypassed',
      file: filename,
      line: lineNumber,
      snippet,
      context: contextLabel,
      detail: `${label}. GitHub Actions registers individual secret values for masking, but when secrets are passed through functions like \`toJSON()\`, \`format()\`, or \`join()\`, the output is a new string that the runner has never seen and therefore never masks. The full secret value appears in plain text in the workflow logs.`,
      ...
      impact: 'Full Secret Exposure in Workflow Run Logs — Masking Defeated',
