...
function makeFinding(filename, lineNumber, snippet, name, level, condition, jobId) {
  return {
    ...
    rule:        'if-always-true',
    severity:    'high',
    title:       `${level} \`if:\` Condition is Always True (Mixed Expression Syntax)`,
    file:        filename,
    line:        lineNumber,
    snippet,
    context:     `${level}: \`${name}\`  ·  Condition: \`${condition}\``,
    detail:      `The \`if:\` condition mixes \`\${{ }}\` interpolation with comparison operators outside the delimiters. GitHub Actions resolves \`\${{ expr }}\` to a string first, then the resulting string — e.g., \`false == 'expected'\` — is re-evaluated as an expression. The left side is an unknown identifier (evaluates to null) or a non-empty string, making the condition always truthy. Any security gate using this syntax is silently bypassed.`,
    exploit:     `If this condition guards a deployment, secret access, or privilege check, the guard never blocks. An attacker triggers the protected step under conditions it was never meant to run in.`,
    impact:      'Security Gate Bypassed — Condition Always True',
    remediation: `Move the entire comparison inside the \`\${{ }}\` delimiters:\n\n# Wrong (always true):\nif: \${{ steps.x.outputs.result }} == 'expected'\n\n# Correct:\nif: \${{ steps.x.outputs.result == 'expected' }}\n\n# Also correct (no ${{ }} needed for if: at all):\nif: steps.x.outputs.result == 'expected'`,
