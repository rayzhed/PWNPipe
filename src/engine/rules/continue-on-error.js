  ...
  for (const [jobId, job] of Object.entries(jobs)) {
    ...
    if (job['continue-on-error'] === true) {
      ...
      findings.push({
        ...
        severity:    'medium',
        title:       `Job \`${jobId}\` Has \`continue-on-error: true\` (All Failures Ignored)`,
        file:        filename,
        line:        lineNumber,
        snippet,
        context:     `Job: \`${jobName}\`  ·  Job-level`,
        detail:      `\`continue-on-error: true\` at the job level makes GitHub Actions report the job as successful regardless of step exit codes. Any failure in this job, including security scan findings, will be ignored and the overall workflow will show green.`,
    ...
    for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
      ...
      findings.push({
        ...
        detail:      `The security scanning step \`${step.uses ?? ''}\` has \`continue-on-error: true\`. Any findings the scanner reports will not block the workflow. High-severity vulnerabilities will be logged but the pipeline will continue to deploy or publish anyway.`,
