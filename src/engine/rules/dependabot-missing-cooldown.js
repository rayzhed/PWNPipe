...
export function checkDependabotMissingCooldown(parsed, rawContent, filePath) {
  ...
  for (let i = 0; i < updates.length; i++) {
    ...
    findings.push({
      ...
      severity: 'low',
      title: `Dependabot: No cooldown for \`${ecosystem}\` updates (${directory})`,
      file: filePath,
      line: lineNumber,
      snippet,
      context: `package-ecosystem: ${ecosystem}  ·  directory: ${directory}`,
      detail: `The \`${ecosystem}\` updates entry has no \`cooldown:\` setting. Without a cooldown, Dependabot will immediately propose any newly published package version. This widens the window for typosquatting attacks and just-compromised packages — where a maintainer account is breached and a malicious version is published — to reach CI before the community has time to detect and yank the package.`,
